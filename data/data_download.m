...
for i=1:length(imgs_names)
    ...
    % try
    fileurl = imgs_names{i}(11:end);
    filepath = [data_folder '/' imgs_names{i}(1:9)];
    if strcmp(fileurl, 'None') == 0
        [abs_filepath, download_status] = urlwrite(fileurl, filepath);
        % [abs_filepath,s] = urlwrite(imgs_names{i}(11:end),[data_folder '/' imgs_names{i}(1:9)]);
        disp(['image:' abs_filepath ' status:' num2str(download_status)]);
    end
end
